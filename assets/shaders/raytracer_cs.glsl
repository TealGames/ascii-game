...
//Will use an exponential moving average instead of a running average.
//Most of the time for offline renderers (ones that only care about image output, not real time rendering)
//running average is recommended because it creates convergence as more samples decreases noise. 
//IF THERE IS A LOT OF UNPREDICTABILITY/MOVEMENT OR IMPRECISION/ERROR, running average is bad as errors from 
//previopus frames accumulate, so using an exponential average gives more weight to new samples and hides errors better
#define USE_EXPONENTIAL_MOVING_AVERAGE 1
...
uint PCGHash(uint hashInput) 
{
    uint state = hashInput * 747796405u + 2891336453u;
...
void main() 
{
    ...
    for (int s = 0; s < uSamplesPerPixel; s++)
    {
        ...
        uint seed = uint(pixel.x * 1973u + pixel.y * 9277u)
          ^ (uUnmovingFrameCount * uSamplesPerPixel + s) * 26699u;
    ...
    averageRadiance /= uSamplesPerPixel;
    ...
    if (uOutputTextureCount >= 1) imageStore(uTextureOutput0, pixel, fragColor);
    if (uOutputTextureCount >= 2) imageStore(uTextureOutput1, pixel, fragColor);
