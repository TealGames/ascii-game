...
namespace Engine::Physics
{
	...
	private:
		...
		SurfacePhysicsProfile m_profile;
		/// <summary>
		...
		const PhysicsWorld* m_physicsSimulation;
	...
	public:
		...
		PhysicsBodyComponent(const CollisionBoxComponent* collisionBox, const float mass);
		PhysicsBodyComponent(const CollisionBoxComponent* collisionBox, const float mass,
			const float gravity, const float terminalYVelocity);

		void SetPhysicsWorldRef(const PhysicsWorld& world);
		void RemovePhysicsWorldRef();
		const PhysicsWorld& GetPhysicsWorldSafe();
		...
		bool HasMass() const;
		const SurfacePhysicsProfile& GetSurfacePhysicsProfile() const;
