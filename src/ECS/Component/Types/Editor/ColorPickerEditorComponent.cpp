...
{
	ColorPickerEditorComponent::ColorPickerEditorComponent() : ColorPickerEditorComponent(UI::UIStyle()) {}
	ColorPickerEditorComponent::ColorPickerEditorComponent(const UI::UIStyle& settings)
	...
	void ColorPickerEditorComponent::Init()
	{
		m_selectable->m_OnSelect.AddListener([this](UI::UISelectableComponent* gui)-> void
			{
				//Assert(false, "PISS");
				//LogWarning(std::format("color picker call addr:{}", ::Utils::ToStringPointerAddress(this)));
				ColorPopupUI* popup = nullptr;
				const bool isEnabled = m_popupManager->TryTogglePopupAt<ColorPopupUI>(GetEntity().
					TryGetComponent<UI::UITransformComponent>()->GetLastGlobalScreenRect(),
					UI::PopupPositionFlags::BelowRect | UI::PopupPositionFlags::CenteredXToRect, &popup);
		...
		m_popupManager->m_OnPopupClosed.AddListener([this](const std::string& type, const UI::PopupUI* popup)-> void
			{
	...
	void ColorPickerEditorComponent::SetValueSetAction(const ColorPickerAction& action)
	{
		m_valueSetCallback = action;
	}

	void ColorPickerEditorComponent::SetColor(const ColHDR4 color)
	{
		m_color = color;
	...
	ColHDR4 ColorPickerEditorComponent::GetColor() const { return m_color; }
	const UI::UIPanelComponent* ColorPickerEditorComponent::GetFieldPanel() const { return m_fieldPanel; }

	void ColorPickerEditorComponent::InitFields()
	{
		m_Fields = {};
	}
	void ColorPickerEditorComponent::Serialize(Serialization::Serializer& serializer) const
	{
	}
	void ColorPickerEditorComponent::Deserialize(Serialization::Deserializer& deserializer)
	{
	}
	std::string ColorPickerEditorComponent::ToString() const
	{
