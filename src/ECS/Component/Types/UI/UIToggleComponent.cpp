...
namespace Engine::UI
{
	UIToggleComponent::UIToggleComponent(const bool& startValue, const UIStyle& settings,
		UITextureComponent* onTexture, UITextureComponent* offTexture, UIPanelComponent* background)//, const TextureAsset* toggledTexture)
	...
	{
		UpdateStyle();
		m_selectable->m_OnClick.AddListener([this](UISelectableComponent* self)-> void
			{
				ToggleValue();
				self->Deselect();
				//SetSettings({});
	...
	void UIToggleComponent::SetStateTextures(UITextureComponent* onTexture, UITextureComponent* offTexture)
	{
	...
	*/
	...
	void UIToggleComponent::Serialize(Serialization::Serializer& serializer) const
	{
	}
	void UIToggleComponent::Deserialize(Serialization::Deserializer& deserializer)
	{
	}
