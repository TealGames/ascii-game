...
UILayoutComponent::UILayoutComponent(const LayoutType type, const SizingType sizing, const NormalizedPos spacing)
	: m_type(type), m_sizingType(sizing), m_spacing(spacing) {}

void UILayoutComponent::AddLayoutElement(EntityData& element)
{
...
std::tuple<EntityData*, UITransformData*> UILayoutComponent::CreateLayoutElement(const std::string& name)
{
...
void UILayoutComponent::LayoutUpdate()
{
	...
	NormalizedPos currentPosNorm = UI_RECT_TOP_LEFT;
...
void UILayoutComponent::Deserialize(const Json& json)
{
	//TODO: implement
	return;
}
Json UILayoutComponent::Serialize()
{
	//TOD: implement
	return {};
}