...
namespace Engine::Math
{
    ...
    Quat Quat::GetNormalized() const
    {
        ...
        if (::Math::ApproximateEqualsF(length, 0)) return Quat(0, 0, 0, 1);
    ...
    {
        const float length = std::sqrt(m_X * m_X + m_Y * m_Y + m_Z * m_Z + m_W * m_W);
        if (::Math::ApproximateEqualsF(length, 0))
        {
            m_X = 0;
            m_Y = 0;
            m_Z = 0;
    ...
    {
        return ToRadians() * ::Math::RAD_TO_DEG_CONSTANT;
    }
    ...
    {
        SetAsRadians(degreeEulerAngle * ::Math::DEG_TO_RAD_CONSTANT);
    }
    ...
    std::string Quat::ToString(const Math::AngleType angleType) const
    {
        if (angleType == Math::AngleType::Euler) return ToRadians().ToString();
