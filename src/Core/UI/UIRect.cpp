...
UIRect::UIRect(const NormalizedPos& size) : UIRect(UI_RECT_BOTTOM_LEFT, size) {}
UIRect::UIRect(const NormalizedPos& bottomLeft, const NormalizedPos& size, const NormalizedPos& pivot) :
...
{
	const NormalizedPos size = GetSize();
	return size.m_X != 0 && size.m_Y != 0;
}

void UIRect::SetSizeWithPivot(const NormalizedPos& size, const NormalizedPos& pivot)
{
...
void UIRect::SetSize(const NormalizedPos& size) { SetSizeWithPivot(size, m_Pivot); }
...
void UIRect::SetBottomLeft(const NormalizedPos& bottomLeft, const bool maintainSize)
{
	const NormalizedPos posDelta = bottomLeft - m_bottomLeftPos;
...
void UIRect::SetBottomRight(const NormalizedPos& bottomRight, const bool maintainSize)
{
	const NormalizedPos posDelta = bottomRight - GetBottomRightPos();
...
void UIRect::SetTopRight(const NormalizedPos& topRight, const bool maintainSize)
{
	const NormalizedPos posDelta = topRight - m_topRightPos;
...
void UIRect::SetTopLeft(const NormalizedPos& topLeft, const bool maintainSize)
{
	const NormalizedPos posDelta = topLeft - GetTopLeftPos();
...
void UIRect::SetBoundsBLTR(const NormalizedPos& bottomLeft, const NormalizedPos& topRight)
{
...
void UIRect::SetBoundsTLBR(const NormalizedPos& topLeft, const NormalizedPos& bottomRight)
{
...
bool UIRect::ContainsPos(const NormalizedPos& pos) const
{
	return m_bottomLeftPos <= pos && pos <= m_topRightPos;
}

NormalizedPos UIRect::GetSize() const { return m_topRightPos - m_bottomLeftPos; }

const NormalizedPos& UIRect::GetBottomLeftPos() const { return m_bottomLeftPos; }
NormalizedPos UIRect::GetTopLeftPos() const { return NormalizedPos(m_bottomLeftPos.m_X, m_topRightPos.m_Y); }
const NormalizedPos& UIRect::GetTopRightPos() const { return m_topRightPos; }
NormalizedPos UIRect::GetBottomRightPos() const { return NormalizedPos(m_topRightPos.m_X, m_bottomLeftPos.m_Y); }

Mat3 UIRect::CalculateModelMatrix() const
{
	return Utils::CalculateUIModelMatrix(nullptr, m_bottomLeftPos.AsVec2(),
...
{
	const NormalizedPos parentSize = parentWorldRect.GetSize();
	const NormalizedPos localSize = localRect.GetSize();

	const NormalizedPos worldBottomLeft = parentWorldRect.GetBottomLeftPos() + localRect.GetBottomLeftPos() * parentSize;
	const NormalizedPos worldSize = parentSize * localSize;
