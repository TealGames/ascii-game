...
namespace Engine::UI
{
	...
	void UIInteractionManager::CreateSelectableArray()
	{
		...
		size_t i = 0;
		m_hierarchy.ElementTraversalDFS<UITransformComponent>([this, &i](UILayer layer, UITransformComponent& element) -> void
			{
				...
				else if (UISelectableComponent* selectable = element.GetEntityMutable().TryGetComponentMutable<UISelectableComponent>())
				{
	...
	void UIInteractionManager::SelectableClickCallback(UISelectableComponent* selectable)
	{
	...
	void UIInteractionManager::SelectableSelectCallback(UISelectableComponent* selectable)
	{
		if (selectable == nullptr)
			return;
	...
	void UIInteractionManager::SelectableHoverStartCallback(UISelectableComponent* selectable)
	{
		if (selectable == nullptr)
			return;
	...
	void UIInteractionManager::StartDrag(UISelectableComponent* selectable)
	{
		if (selectable == nullptr)
			return;
	...
	void UIInteractionManager::AddSelectable(const UILayer layer, UISelectableComponent* selectable)
	{
		if (selectable == nullptr)
		{
		...
		if (it == m_selectableLayers.end())
			m_selectableLayers.emplace(layer, std::vector<UISelectableComponent*>{ selectable});
		...
		selectable->m_OnClick.AddListener([this](UISelectableComponent* selectable)-> void {SelectableClickCallback(selectable); });
		selectable->m_OnSelect.AddListener([this](UISelectableComponent* selectable)-> void {SelectableSelectCallback(selectable); });
		selectable->m_OnDeselect.AddListener([this](UISelectableComponent* selectable)-> void
			{
				if (selectable != m_currentSelected) return;
				m_currentSelected = nullptr;
			});
		selectable->m_OnHoverStart.AddListener([this](UISelectableComponent* selectable)-> void {SelectableHoverStartCallback(selectable); });
		selectable->m_OnHoverEnd.AddListener([this](UISelectableComponent*)-> void { m_currentHovered = nullptr; });

		//Note: drag does not have reserved start and end since it is 
	}
	void UIInteractionManager::AddSelectables(const UILayer layer, const std::vector<UISelectableComponent*>& selectables)
	{
		for (const auto& selectable : selectables)
			AddSelectable(layer, selectable);
	}

	void UIInteractionManager::AddSelectionEventBlocker(const UILayer layer, const size_t elementIndex, const UITransformComponent& element)
	{
		auto it = m_selectableLayers.find(layer);
		...
		if (it == m_selectableLayers.end())
			m_selectableLayers.emplace(layer, std::vector<UISelectableComponent*>{ nullptr});
	...
	const UISelectableComponent* UIInteractionManager::TryGetSelectableSelected() const { return m_currentSelected; }
	const UISelectableComponent* UIInteractionManager::TryGetSelectableDragged() const { return m_currentDragged; }
	const UISelectableComponent* UIInteractionManager::TryGetSelectableHovered() const { return m_currentHovered; }
