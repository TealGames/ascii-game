	...
	m_topLeftPos(topLeft), m_bottomRightPos(GetBottomRight(topLeft, size)) 
{
	if (HasInvalidSize())
	{
		LogError(std::format("Tried to create a relative GUI rect with invalid size. "
...
void RelativeUIRect::SetSize(const NormalizedPosition& size)
{
	...
	if (HasInvalidSize())
	{
		LogError(std::format("Tried to set size for relative "
			...
			size.ToString(), ToString(), originalRect.ToString(), 
			GetBottomRight(NormalizedPosition::TOP_LEFT, size).ToString(), netAvailableSpace.ToString()));
	}
}
...
void RelativeUIRect::SetTopLeft(const NormalizedPosition& topLeft)
{
	...
	if (HasInvalidSize())
	{
		LogError(std::format("Tried to set top left pos for relative "
...
void RelativeUIRect::SetBottomRight(const NormalizedPosition& bottomRight)
{
	...
	if (HasInvalidSize())
	{
		LogError(std::format("Tried to set bottom right pos for relative "
			"gui rect but it resulted in invalid size:{}", ToString()));
	}
}
void RelativeUIRect::SetBounds(const NormalizedPosition& topLeft, const NormalizedPosition& bottomRight)
{
	...
	if (HasInvalidSize())
	{
		LogError(std::format("Tried to set bounds:[TopLeft:{} BottomRight:{}] for relative "
