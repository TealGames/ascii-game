...
namespace Engine
{
	...
	void Profiler::UpdateProfilerSummary(const ProfilerProcess& process)
	{
		...
		if (process.m_MicrosecondsTime < routine.m_FastestTime ||
			::Math::ApproximateEqualsF(routine.m_FastestTime, 0))
		{
		...
		if (!m_profilerSummary.m_FastestProcess.has_value() ||
			::Math::ApproximateEqualsF(m_profilerSummary.m_FastestProcess.value().m_MicrosecondsTime, 0) ||
	...
	void Profiler::LogCurrentRoundTimes()
	{
		...
		std::cout << std::format("\n{}{}{}", ANSI_COLOR_GREEN, log, ANSI_COLOR_CLEAR) << std::endl;
	}
