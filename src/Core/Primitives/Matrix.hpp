...
namespace Engine
{
	...
	using MatrixRow = Vec<T, SIZE, ORDER_ALIGN_BYTES>;
	...
	struct MatrixStorage<T, ROW_SIZE, COL_SIZE, MatrixMajorOrder::Row, ORDER_ALIGN_BYTES>
	{
		...
		using Type = MatrixRow<T, COL_SIZE, ORDER_ALIGN_BYTES>[ROW_SIZE];
	};
	...
	struct MatrixStorage<T, ROW_SIZE, COL_SIZE, MatrixMajorOrder::Column, ORDER_ALIGN_BYTES>
	{
		...
		using Type = MatrixRow<T, ROW_SIZE, ORDER_ALIGN_BYTES>[COL_SIZE];
	};

	template<size_t ROW_SIZE, size_t COL_SIZE>
	concept IsPositiveRowColSize = (ROW_SIZE > 0 && COL_SIZE > 0);

	//TODO: add other matrix operations like inversion, row swapping, gausian elimination, determinant

	template<typename T, size_t ROW_SIZE, size_t COL_SIZE, MatrixMajorOrder ORDER, size_t ORDER_ALIGN_BYTES = alignof(T)>
		requires (IsPositiveRowColSize<ROW_SIZE, COL_SIZE> && std::is_arithmetic_v<T>)
	/// <summary>
	...
	public:
		...
		template<size_t OTHER_ROW_SIZE, size_t OTHER_COL_SIZE, MatrixMajorOrder OTHER_ORDER, size_t OTHER_ALIGN>
			requires (IsPositiveRowColSize<OTHER_ROW_SIZE, OTHER_COL_SIZE> && OTHER_ROW_SIZE <= ROW_SIZE && OTHER_COL_SIZE <= COL_SIZE)
		...
		{
			if constexpr (ORDER == MatrixMajorOrder::Row)
				return sizeof(MatrixRow<T, ROW_SIZE, ORDER_ALIGN_BYTES>);
			else return sizeof(MatrixRow<T, COL_SIZE, ORDER_ALIGN_BYTES>);
		}
		...
		bool IsAllZero() const
		{
			for (size_t r = 0; r < ROW_SIZE; r++)
			{
				for (size_t c = 0; c < COL_SIZE; c++)
				{
					if (!::Math::ApproximateEqualsF(GetUnsafe(r, c), 0.0f))
						return false;
				}
			}
			return true;
		...
		bool HasValue(const T& val) const
		{
			for (size_t r = 0; r < ROW_SIZE; r++)
			{
				for (size_t c = 0; c < COL_SIZE; c++)
				{
					if (::Math::ApproximateEqualsF(GetUnsafe(r, c), val))
						return true;
				}
			}
			return false;
		...
		bool IsOrthonormal() const requires (ROW_SIZE == 3 && COL_SIZE == 3)
		{
			...
			if (!::Math::ApproximateEqualsF(DotProduct(basisX, basisY), 0) ||
				!::Math::ApproximateEqualsF(DotProduct(basisY, basisZ), 0) ||
				!::Math::ApproximateEqualsF(DotProduct(basisX, basisZ), 0))
			{
				return false;
			}

			return ::Math::ApproximateEqualsF(CalculateDeterminant(), 1);
		}
...
#pragma region GetFunctions
		...
		template<size_t SLICE_ROW_SIZE, size_t SLICE_COL_SIZE>
			requires (IsPositiveRowColSize<SLICE_ROW_SIZE, SLICE_COL_SIZE>
...
#pragma region MathOperations
		...
		template<size_t OTHER_ROW_SIZE, size_t OTHER_COL_SIZE, MatrixMajorOrder OTHER_ORDER, size_t OTHER_ALIGN>
			requires (IsPositiveRowColSize<OTHER_ROW_SIZE, OTHER_COL_SIZE>&& COL_SIZE == OTHER_ROW_SIZE)
