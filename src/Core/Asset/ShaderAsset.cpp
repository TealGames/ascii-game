...
namespace Engine::Rendering
{
	...
	const std::array<std::string_view,1> ShaderAsset::EXTENSIONS = {".glsl"};

	ShaderAsset::ShaderAsset(const std::filesystem::path& path)
		: Asset(path), m_shader(Rendering::CreateShader("", ""))
	{
	...
	void ShaderAsset::WriteToShaderFromFiles()
	{
		...
		Assets::AssetManager::SetAssetHiddenStatus(otherShaderPath, true);

		const std::string thisShaderSource = ::Utils::IO::TryReadFileFull(GetAbsolutePath());
		const std::string otherShaderSource = ::Utils::IO::TryReadFileFull(otherShaderPath);
	...
	void ShaderAsset::ReadShaderFromSingleFile(const Rendering::ShaderProgramType programType)
	{
		...
		Utils::IO::TryExecuteOnFileByLine(GetAbsolutePath(),
