...
void SceneAsset::SaveToPath(const std::filesystem::path& path)
{
	...
	{
		//LogWarning(std::format("Entity:{} has components:{} serializabe:{}", entity->m_Name, entity->GetAllComponentsMutable().size(), entity->m_IsSerializable));
	...
	//LogError(std::format("Resulting json : {} entity count:{}", JsonUtils::ToStringProperties(json), scene.GetLocalEntitiesMutable().size()));
	
	if (json.empty())
	{
		LogWarning(std::format("Failed to save scene asset:{} to path:{} "
			"because resulting json was null", ToString(), path.string()));
		return;
	}
