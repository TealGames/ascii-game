...
namespace Engine::Rendering::Backend
{
	...
	namespace OpenGl = Engine::Rendering::OpenGl;
	...
	void LoadBackend()
	{
		...
		BackendLoaded = true;
		Core::EngineLog("LOADED GRAPHICS BACKEND:{}", GetBackendVersion());
	}
...
#error "No rendering library is active"
	...
	void ClearBackground(std::uint8_t clearColorAttachments, const ColHDR4 clearColor, const const float clearDepth)
	{
#if defined(OPENGL)
