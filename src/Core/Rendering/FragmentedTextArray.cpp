...
namespace Engine::Rendering
{
	...
	TextCharArrayPosition* FragmentedTextArray::TryGetPosMutable(const Array2DPosition& rowColPos, const TextArrayPositionType& positionType)
	{
		...
		{
			if (!Assert(IsValidIndexPos(rowColPos), "Tried to get pos MUTABLE from row col index: {} "
				"but it is out of bounds", rowColPos.ToString()))
				return nullptr;
	...
	void FragmentedTextArray::SetAt(const Array2DPosition& rowColPos, const TextArrayPositionType& positionType, const TextChar& newBufferChar)
	{
		...
		if (!Assert(pos != nullptr, "Tried to set position: {}({}) with BUFFER CHAR: {} for fragmented text array "
			"but data was not found", rowColPos.ToString(), Rendering::ToString(positionType), newBufferChar.ToString()))
			return;
	...
	{
		TextCharArrayPosition* pos = TryGetPosMutable(rowColPos, positionType);
		if (!Assert(pos != nullptr, "Tried to set position: {}({}) with CHAR: {} for fragmented text array "
			"but data was not found", rowColPos.ToString(), Rendering::ToString(positionType), ::Utils::ToString(newChar)))
			return;

		pos->m_Text.SetChar(newChar);
	}
	void FragmentedTextArray::SetAt(const Array2DPosition& rowColPos, const TextArrayPositionType& positionType, const ColHDR4& newColor)
	{
		TextCharArrayPosition* pos = TryGetPosMutable(rowColPos, positionType);
		if (!Assert(pos != nullptr, "Tried to set position: {}({}) with COLOR: {} for fragmented text array "
			"but data was not found", rowColPos.ToString(), Rendering::ToString(positionType), newColor.ToString()))
			return;
	...
	void FragmentedTextArray::SetAt(const std::vector<Array2DPosition>& rowColPos, const TextArrayPositionType& positionType, const TextChar& newBufferChar)
	{
		...
		for (const auto& pos : rowColPos)
		{
			...
			if (!Assert(dataPtr != nullptr, "Tried to set position: {}({}) OF OTHER POSITIONS with buffer: {} for fragmented text array "
				"but data was not found", pos.ToString(), Rendering::ToString(positionType), newBufferChar.ToString())) return;
	...
	void FragmentedTextArray::SetAt(const std::vector<TextCharArrayPosition>& updatedCharsAtPos, const TextArrayPositionType& positionType)
	{
		...
		for (const auto& updatedChars : updatedCharsAtPos)
		{
			...
			if (!Assert(dataPtr != nullptr, "Tried to set position: {}({}) OF OTHER POSITIONS with text buffer: {} for fragmented text array "
				"but data was not found", updatedChars.m_RowColPos.ToString(), Rendering::ToString(positionType), updatedChars.ToString())) return;
	...
	void FragmentedTextArray::SetAt(const std::vector<ColorPosition>& updateColorsAtPos, const TextArrayPositionType& positionType)
	{
		...
		{
			dataPtr = TryGetPosMutable(updatedChars.m_RowColPos, positionType);
			if (!Assert(dataPtr != nullptr, "Tried to set position: {}({}) OF OTHER POSITIONS with text color: {} for fragmented text array "
				"but data was not found", updatedChars.m_RowColPos.ToString(), Rendering::ToString(positionType), updatedChars.ToString())) return;
		...
		const bool convertAll)
	{
		...
		for (int r = 0; r < chars.size(); r++)
		{
			...
			{
				currentChar = chars[r][c].m_Text.GetChar();
