...
void FragmentedTextArray::SetAt(const Array2DPosition& rowColPos, const TextArrayPositionType& positionType, const TextChar& newBufferChar)
{
	...
	if (!Assert(pos != nullptr, "Tried to set position: {}({}) with BUFFER CHAR: {} for fragmented text array "
		"but data was not found", rowColPos.ToString(), ::ToString(positionType), newBufferChar.ToString())) 
		return;
...
void FragmentedTextArray::SetAt(const Array2DPosition& rowColPos, const TextArrayPositionType& positionType, const char& newChar)
{
	...
	if (!Assert(pos != nullptr, "Tried to set position: {}({}) with CHAR: {} for fragmented text array "
		"but data was not found", rowColPos.ToString(), ::ToString(positionType), Utils::ToString(newChar))) 
		return;
...
void FragmentedTextArray::SetAt(const Array2DPosition& rowColPos, const TextArrayPositionType& positionType, const Color& newColor)
{
	...
	if (!Assert(pos != nullptr, "Tried to set position: {}({}) with COLOR: {} for fragmented text array "
		"but data was not found", rowColPos.ToString(), ::ToString(positionType), newColor.ToString())) 
		return;
