...
namespace Engine
{
	...
	struct alignas(ALIGN) BVHNodeBase
	{
		...
		bool IsIntersectedByRay(const Math::Ray3D& ray, float* outMargin) const
		{
			return Math::RayIntersectsBounds<ALIGN>(m_Bounds, ray, outMargin);
		}
		bool IsIntersectedByRay(const Math::Ray3D& ray, float* outTEnter, float* outTExit) const
		{
			return Math::RayIntersectsBounds<ALIGN>(m_Bounds, ray, outTEnter, outTEnter);
		}
	...
	{
		return BVHNodeBase<NEW_ALIGN>(Engine::ConvertAlignment<START_ALIGN, NEW_ALIGN>(node.m_Bounds),
			node.m_ObjectStartIndex, node.m_ObjectCount, node.m_IndexChild0, node.m_IndexChild1);
	}

	using BVHNode = BVHNodeBase<alignof(float)>;
	...
	template<typename T, size_t NODE_ALIGN = 0>
	bool IntersectsBVH(const Math::Ray3D& ray, const BVHNodeBase<NODE_ALIGN>* rootNode, const size_t nodeCount,
		...
		const std::function<bool(const BVHNodeBase<NODE_ALIGN>& node, const T& obj,
			const Math::Ray3D& ray, float* outHitDistance)>& objectIntersectedByRayFunc)
	{
		ENGINE_ASSERT(Math::IsUnitVector(ray.m_Dir), "Attempted to invoke IntersectsBVH with non-unit vector dir: {}", ray.m_Dir.ToString());
		...
		const std::function<bool(const BVHNodeBase<NODE_ALIGN>&)>& leafSuccessorIsValidFunc, bool outputMessages)
	{
		...
		if (parentNode != nullptr)
		{
			...
			if (!Math::IsWithinBounds(parentBounds, thisBounds.m_MaxPos) ||
				!Math::IsWithinBounds(parentBounds, thisBounds.m_MinPos))
			{
				if (outputMessages)
				{
		...
		if (currentNode.IsLeaf())
		{
			for (size_t i = 0; i < currentNode.m_ObjectCount; i++)
			{
				...
				if (!Math::IsWithinBounds(thisBounds, objectBounds.m_MaxPos) ||
					!Math::IsWithinBounds(thisBounds, objectBounds.m_MinPos))
				{
					if (outputMessages)
					{
	...
	/// </summary>
	template<typename T, size_t NODE_ALIGN = alignof(T)>
	...
	private:
				...
				int& outStartIndex, std::uint32_t& outObjectCount)>& objectIntervalOverrideFunc)
		{
			...
			for (i = 1; i < objectCount; i++)
			{
				...
				centerBoundsMin = Math::Min(centerBoundsMin, triangleCenter);
				centerBoundsMax = Math::Max(centerBoundsMax, triangleCenter);
			}
			...
			else if (splitAlgorithm == BVHSplitAlgorithm::SurfaceArea)
			{
				...
				for (i = 1; i < objectCount - 1; i++)
				{
					const float currentCost = Math::CalculateSurfaceArea(prefixBounds[i - 1]) * i +
						Math::CalculateSurfaceArea(suffixBounds[i]) * (objectCount - i);
	...
	private:
			...
			const size_t leafObjectCount, const BVHSplitAlgorithm splitAlgorithm)
		{
			...
			else if (splitAlgorithm == BVHSplitAlgorithm::Median) worseCaseNodes = 2 * ::Math::NextPowerOf2(objectCount / leafObjectCount) - 1;
	...
	public:
		...
		bool Intersects(const Math::Ray3D& ray, OutT* objectArray, const OutT** outHitObject, float* outMinHitDistance,
			const std::function<bool(const NodeType& node, const OutT& obj,
				const Math::Ray3D& ray, float* outHitDistance)>& objectIntersectedByRayFunc) const
		{
			return Engine::IntersectsBVH<OutT>(ray, &m_flatNodes[0], m_flatNodes.size(),
				objectArray, m_objectIndices.empty() ? nullptr : &m_objectIndices[0], outHitObject, outMinHitDistance, objectIntersectedByRayFunc);
		}

		template<typename OutT>
			requires HasNamedFunctionIsIntersectedByRay<OutT, bool, Math::Ray3D, float*>
		bool Intersects(const Math::Ray3D& ray, OutT* objectArray, const OutT** outHitObject) const
		{
			return Intersects<OutT>(ray, objectArray, outHitObject,
				[](const OutT& obj, const Math::Ray3D& ray, float* outHitDistance) -> bool
				{
					return obj.IsIntersectedByRay(ray, outHitDistance);
				});
		}

		bool Intersects(const Math::Ray3D& ray, const T** outHitObject, float* outMinHitDistance) const
			requires HasNamedFunctionIsIntersectedByRay<T, bool, Math::Ray3D, float*>
		{
