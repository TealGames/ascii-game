...
namespace Engine::Rendering
{
	...
	VisualDataPreset::VisualDataPreset(const FontAsset& font, const float& fontSize, const Vec2& charSpacing,
		const CharAreaType& charAreaType, const Vec2& predefinedCharArea, const NormalizedVec2& relativePivotPos) :
	...
	VisualData::VisualData() : VisualData({}, Vec2(), NormalizedVec2()) {}


	VisualData::VisualData(const FragmentedTextBuffer2D& rawBuffer, const NormalizedVec2& relativePivotPos) :
	...
	VisualData::VisualData(const std::vector<std::vector<TextChar>>& rawBuffer, const Vec2& charSpacing,
		const WorldFontProperties& fontSettings, const NormalizedVec2& relativePivotPos) : m_buffer(), m_pivotRelative(relativePivotPos), m_worldSize()
	{
		...
		const WorldFontProperties& fontSettings, const NormalizedVec2& relativePivotPos) : m_buffer(), m_pivotRelative(relativePivotPos), m_worldSize()
	{
		...
		Vec2 pivotDiff = {};
		NormalizedVec2 currPosNormalized = SPRITE_TOP_LEFT;
	...
	void VisualData::AddToCreatedBuffer(const size_t& r, const size_t& c, const size_t currRowElementCount, const TextChar& textChar, const WorldFontProperties& fontData, const Vec2& charSpacing,
		Vec2& pivotDiff, NormalizedVec2& currPosNormalized, const Vec2& fullSize, float* currentRowMaxHeight, const Vec2* predefinedCharArea)
	{
	...
	*/
	...
	std::string VisualData::ToString() const
	{
		return std::format("[Visual Size:{} Pivot:{} Text:{}]",
			GetWorldSize().ToString(), m_pivotRelative.AsVec2().ToString(), Rendering::ToString(m_buffer));
	}
