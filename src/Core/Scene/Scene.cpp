...
namespace Engine::Scenes
{
	...
	Scene::Scene(const std::string& sceneName, GlobalEntityManager* manager) :
		m_layers{}, m_sceneName(sceneName), m_localRootEntities(),
		...
		m_globalEntities(manager)
	{
		...
		m_layers.emplace(Rendering::RenderLayerType::Background, Rendering::RenderLayer{});
		m_layers.emplace(Rendering::RenderLayerType::Player, Rendering::RenderLayer{});
		m_layers.emplace(Rendering::RenderLayerType::UI, Rendering::RenderLayer{});
	...
	std::vector<Rendering::RenderLayer*> Scene::GetLayersMutable()
	{
		if (m_layers.empty()) return {};

		std::vector<Rendering::RenderLayer*> layers = {};
		for (auto& layer : m_layers) layers.push_back(&(layer.second));
		return layers;
	}
	std::vector<Rendering::RenderLayer*> Scene::GetLayersMutable(const Rendering::RenderLayerType& renderLayers)
	{
		if (m_layers.empty()) return {};
	...
	std::vector<const Rendering::RenderLayer*> Scene::GetLayers(const Rendering::RenderLayerType& renderLayers) const
	{
		if (m_layers.empty()) return {};

		std::vector<const Rendering::RenderLayer*> layers = {};
	...
	std::vector<const Rendering::RenderLayer*> Scene::GetAllLayers() const
	{
		if (m_layers.empty()) return {};
	...
	void Scene::AddToLayer(const Rendering::RenderLayerType& layers, Rendering::TextBufferCharPosition2D& textBufferPos)
	{
		if (layers == Rendering::RenderLayerType::None) return;
	...
	std::vector<Rendering::FragmentedTextBuffer2D*> Scene::GetLayerBufferMutable(const Rendering::RenderLayerType& renderLayers)
	{
		std::vector<Rendering::FragmentedTextBuffer2D*> buffers = {};
		if (renderLayers == Rendering::RenderLayerType::None) return buffers;
	...
	std::vector<std::tuple<Rendering::RenderLayerType, Rendering::FragmentedTextBuffer2D*>> Scene::GetAllLayerBufferMutable()
	{
		std::vector<std::tuple<Rendering::RenderLayerType, Rendering::FragmentedTextBuffer2D*>> buffers = {};
		for (auto& layer : m_layers)
		{
	...
	std::vector<const ECS::EntityData*> Scene::GetAllEntities() const
	{
		std::vector<const ECS::EntityData*> entities = GetLocalEntities();
	...
	std::vector<const ECS::EntityData*> Scene::GetLocalEntities() const
	{
		std::vector<const ECS::EntityData*> entities = {};
		for (const auto& localEntity : m_localRootEntities)
			entities.push_back(localEntity);

		return entities;
	}
	std::vector<ECS::EntityData*> Scene::GetLocalEntitiesMutable()
	{
		std::vector<ECS::EntityData*> entities = {};
	...
	std::vector<ECS::EntityData*> Scene::GetAllEntitiesMutable()
	{
		std::vector<ECS::EntityData*> entities = {};
		for (auto& entity : TryGetGlobalEntityManagerMutable().GetAllGlobalEntitiesMutable())
		{
	...
	ECS::EntityData& Scene::CreateEntity(const std::string& name, const TransformComponent& transform)
	{
		ECS::EntityData& createdEntity = m_registry.CreateNewEntity(name, transform);
	...
	ECS::EntityData* Scene::TryGetEntityMutable(const ECS::EntityID& id)
	{
		...
		ECS::EntityData* localEntity = m_registry.TryGetEntityMutable(id);
	...
	ECS::EntityData* Scene::TryGetEntityMutable(const std::string& name, const bool& ignoreCase)
	{
	...
	const ECS::EntityData* Scene::TryGetEntity(const std::string& name, const bool& ignoreCase) const
	{
