...
namespace Engine::Editor::UI
{
	...
	void ColorChannelUI::CreateChannel(UI::UITransformComponent& parent, const Input::InputManager& input)
	{
		//m_Text())
		...
		m_Slider = &(containerEntity->AddComponent(UI::UISliderComponent(Vec2{ 0, 1 }, Styles::GetSliderStyle())));
		m_Text = &(containerEntity->AddComponent(UI::UITextComponent("", Styles::GetTextStyleFactorSize(UI::TextAlignment::Center))));

		m_Text->GetEntityMutable().TryGetComponentMutable<UI::UITransformComponent>()->SetLocalBoundsTLBR(UI::UI_RECT_TOP_LEFT, { CHANNEL_TEXT_WIDTH, 0 });
		m_Slider->GetEntityMutable().TryGetComponentMutable<UI::UITransformComponent>()->SetLocalBoundsTLBR(
	...
	{
		m_hexField = &(m_Container->GetEntityMutable().AddComponent(UI::UIInputFieldComponent(UI::InputFieldType::Any, UI::InputFieldFlag::None,
			Styles::GetInputFieldStyle(UI::TextAlignment::Center, INPUT_FIELD_TEXT_FONT_FACTOR))));

		m_Container->SetLocalSize({ 0.4, 0.3 });
		m_sliderLayout = &(m_Container->GetEntityMutable().AddComponent(UI::UILayoutComponent(UI::LayoutType::Vertical, UI::SizingType::ShrinkOnly, { 0, 0.1 })));
		m_sliderLayout->GetEntityMutable().TryGetComponentMutable<UI::UITransformComponent>()->SetLocalBoundsTLBR({ 0, SLIDER_LAYOUT_SIZE }, { 1, 0 });

		m_hexField->GetEntityMutable().TryGetComponentMutable<UI::UITransformComponent>()->
			SetLocalBoundsTLBR({ COLOR_DISPLAY_RADIUS * 3, 1 }, { 1, SLIDER_LAYOUT_SIZE * 1.5 });
	...
	void ColorPopupUI::SetColor(const ColHDR4 color)
	{
		m_rgbChannels[0].SetValue(color.m_R);
		m_rgbChannels[1].SetValue(color.m_G);
		m_rgbChannels[2].SetValue(color.m_B);
	}
	ColHDR4 ColorPopupUI::GetColor() const
	{
		return ColHDR4(m_rgbChannels[0].GetValue(), m_rgbChannels[1].GetValue(),
