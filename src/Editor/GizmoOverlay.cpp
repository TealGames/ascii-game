...
namespace Engine::Editor::Debug
{
	...
	static constexpr ColHDR4 COLLIDER_OUTLINE_COLOR = COLOR_GREEN;
	static constexpr ColHDR4 UI_RECT_OUTLINE_COLOR = COLOR_YELLOW;

	GizmoOverlay::GizmoOverlay(MainUI::UIRenderSystem& renderSystem, Physics::PhysicsManager& physicsManager, const Camera::CameraController& cameraController)
		: m_queuedCalls(), m_activeGizmos(GizmoType::None)
	{
		renderSystem.m_OnElementProcessed.AddListener([this](const UI::UIRendererComponent* renderer, UI::UIRect* rect)-> void
			{
		...
		physicsManager.GetPhysicsWorldMutable().m_OnObjectProcessed.AddListener([this, &cameraController](Physics::PhysicsBodyComponent* body)-> void
			{
