...
template<typename T>
Vec<T, 3> operator/(T lhs, const Vec<T, 3>& rhs)
{
	return Vec<T, 3>(lhs / rhs.m_X, lhs / rhs.m_Y, lhs / rhs.m_Z);
}

template<typename T>
Vec<T, 3> operator/(int lhs, const Vec<T, 3>& rhs) requires (!std::is_same_v<T, int>)
{
	return Vec<T, 3>(lhs / rhs.m_X, lhs / rhs.m_Y, lhs / rhs.m_Z);
}
...
template<typename T>
Vec<T, 3> Max(const Vec<T, 3>& vec1, const Vec<T, 3>& vec2)
{
	return Vec<T, 3>(std::max(vec1.m_X, vec2.m_X), std::max(vec1.m_Y, vec2.m_Y), std::max(vec1.m_Z, vec2.m_Z));
}

template<typename T>
Vec<T, 3> Min(const Vec<T, 3>& vec1, const Vec<T, 3>& vec2)
{
	return Vec<T, 3>(std::min(vec1.m_X, vec2.m_X), std::min(vec1.m_Y, vec2.m_Y), std::min(vec1.m_Z, vec2.m_Z));
}
