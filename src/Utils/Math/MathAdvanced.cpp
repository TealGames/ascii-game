...
namespace Utils
{
	...
	{
		ENGINE_ASSERT(ray.m_Dir.IsUnitVector(), "Attempted to invoke RayIntersectsTriangle but ray dir is not unit vector:{}", ray.m_Dir.ToString());
	...
	{
		ENGINE_ASSERT(ray.m_Dir.IsUnitVector(), "Attempted to invoke RayIntersectsSphere but ray dir is not unit vector:{}", ray.m_Dir.ToString());
