...
struct alignas(ALIGN_BYTES) Vec
{
	...
	{
		if (index >= N)
			throw std::invalid_argument(std::format("Invalid vec index:{}", index));
...
template<typename T, size_t N>
T MaxVal(const Vec<T, N>& vec)
{
	T max = vec.m_Components[0];
	for (std::uint8_t i = 1; i < N; i++)
	{
		max = std::max(max, vec.m_Components[i]);
	}
	return max;
}
...
template<typename T, size_t N>
T MinVal(const Vec<T, N>& vec)
{
	T min = vec.m_Components[0];
	for (std::uint8_t i = 1; i < N; i++)
	{
		min = std::min(min, vec.m_Components[i]);
	}
	return min;
}