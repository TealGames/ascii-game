...
UIRect::UIRect() : UIRect(ScreenPosition(), Vec2()) {}

UIRect::UIRect(const ScreenPosition& topLeftPos, const Vec2& size) :
...
ScreenPosition UIRect::GetPosWithinRect(const NormalizedPosition& normalizedPos) const
{
	...
	return ScreenPosition{m_TopLeftPos.m_X+ normalizedPos.GetPos().m_X * size.m_X,
			//We add the section from the top left y because Y POS INCREASES as you move DOWN
			m_TopLeftPos.m_Y+ (1- normalizedPos.GetPos().m_Y)* size.m_Y};
}
...
Vec2 UIRect::GetSize() const
{
	...
	return Vec2(m_BottomRightPos.m_X - m_TopLeftPos.m_X, m_BottomRightPos.m_Y- m_TopLeftPos.m_Y);
}
...
void UIRect::SetSize(const Vec2& size)
{
	m_BottomRightPos = m_TopLeftPos + Vec2(size.m_X, -size.m_Y);
}
