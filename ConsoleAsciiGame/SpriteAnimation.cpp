...
SpriteAnimation::SpriteAnimation(const std::string& name, const std::vector<SpriteAnimationFrame>& frames,
	...
	m_Name(name), m_FrameDeltas{}, m_FrameVisuals(), m_FrameIndex(0), m_SingleLoopLength(loopTime),
	m_NormalizedTime(0), m_AnimationSpeed(animationSpeed), m_Loop(loop)
{
	SetVisualsFromFrames(frames);
}

const VisualData& SpriteAnimation::GetVisualDataForFrame(const size_t index) const
{
	if (!Assert(this, 0 <= index && index < m_FrameVisuals.size(), std::format("Tried to get visual data for frame "
		"at invalid index:{} but index is valid in range:[0,{})", std::to_string(index), std::to_string(m_FrameVisuals.size()))))
		throw std::invalid_argument("Invalid frame index");

	return m_FrameVisuals.at(index);
}
const VisualData* SpriteAnimation::TryGetCurrentVisualData() const
{
	if (m_FrameIndex == -1 || m_FrameIndex >= m_FrameDeltas.size()) 
		return nullptr;

	return &(GetVisualDataForFrame(m_FrameIndex));
}

const SpriteAnimationDelta* SpriteAnimation::TryGetCurrentAnimationDelta() const
{
	if (m_FrameIndex == -1 || m_FrameIndex >= m_FrameDeltas.size()) 
		return nullptr;
	
	return &(m_FrameDeltas[m_FrameIndex]);
}
...
void SpriteAnimation::SetVisualsFromFrames(const std::vector<SpriteAnimationFrame>& frames)
{
	...
	for (int i = 0; i < frames.size(); i++)
	{
		...
		m_FrameVisuals.push_back(*currentData);
...
{
	return std::format("[SpriteAnimation Loop:{} Speed:{} Length:{} VisualDeltas:{}]",
		std::to_string(m_Loop), std::to_string(m_AnimationSpeed), std::to_string(m_SingleLoopLength),
		Utils::ToStringIterable<std::vector<SpriteAnimationDelta>, SpriteAnimationDelta>(m_FrameDeltas));
}