...
namespace Conversions
{
	...
	ScreenPosition WorldToScreenPosition(const CameraData& camera, const WorldPosition& pos)
	{
		...
		Utils::Point2D screenPercent{ (pos.m_X - bottomLeftX) / camera.m_CameraSettings.m_WorldViewportSize.m_X,
	...
	WorldPosition ScreenToWorldPosition(const CameraData& camera, const ScreenPosition& pos)
	{
		...
		Utils::Point2D screenPercent{ static_cast<float>(pos.m_X) / SCREEN_WIDTH, static_cast<float>(SCREEN_HEIGHT- pos.m_Y) / SCREEN_HEIGHT};

		Utils::Point2D worldSize = screenPercent * camera.m_CameraSettings.m_WorldViewportSize;
		return bottomLeftPos + worldSize;
	}

	ScreenPosition NormalizedScreenToPosition(const NormalizedPosition& normalized)
	{
		Utils::Point2D screenValues = { normalized.GetPos().m_X* SCREEN_WIDTH, normalized.GetPos().m_Y * SCREEN_HEIGHT};
